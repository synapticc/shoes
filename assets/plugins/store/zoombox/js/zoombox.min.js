...
<img class='zoombox-image slide' src='${clientRootDir}/build/images/load.svg'>
...
<img class='zoombox-close zoombox-controls' title="Close" src='${clientRootDir}/build/images/close.png'>
<img class='zoombox-next zoombox-controls' title="Next" src='${clientRootDir}/build/images/next.png'>
<img class='zoombox-prev zoombox-controls' title="Previous" src='${clientRootDir}/build/images/prev.png'>
<img class='zoombox-play zoombox-controls'title="Play" src='${clientRootDir}/build/images/play.png'>
<img class='zoombox-zoom-in zoombox-controls' title="Zoom in" src='${clientRootDir}/build/images/zoom-in.svg'>
<img class='zoombox-zoom-out zoombox-controls' title="Zoom out" src='${clientRootDir}/build/images/zoom-out.svg'>
...
checkPlay(isplay){this.zoomboxImage.style.transform=` translate(-50%,-50%) scale(1)`;if(isplay){this.isplaying=!1;this.zoomboxPlay.src=`${clientRootDir}/build/images/play.png`;clearTimeout(this.slideTimeOut)}else{this.isplaying=!0;this.zoomboxPlay.src=`${clientRootDir}/build/images/pause.png`;this.slideShow()}}
...
var zoombox=new zoomboxImages;export default function zoomBoxExecute(){const imageGroup=document.querySelectorAll('[zoombox]');var groupName=[];imageGroup.forEach((image)=>{let att=image.getAttribute('zoombox');if(att==""){image.setAttribute('zoombox','default');att='default'}
if(!(groupName.includes(att)))
groupName.push(att)})
groupName.forEach((group)=>{var images=document.querySelectorAll(`[zoombox=${group}]`);images.forEach((img,index)=>{img.onclick=()=>{var groupName=img.getAttribute('zoombox');zoombox=new zoomboxImages(`[zoombox=${groupName}]`,img.src,img.getAttribute('src2'),img.alt,index);zoombox.execute();document.querySelector('.zoombox-close').src=`${clientRootDir}/build/images/close.png`;document.querySelector('.zoombox-zoom-in').src=`${clientRootDir}/build/images/zoom-in.svg`;document.querySelector('.zoombox-zoom-out').src=`${clientRootDir}/build/images/zoom-out.svg`;document.querySelector('.zoombox-next').src=`${clientRootDir}/build/images/next.png`;document.querySelector('.zoombox-prev').src=`${clientRootDir}/build/images/prev.png`;document.querySelector('.zoombox-play').src=`${clientRootDir}/build/images/play.png`}})})}
zoomBoxExecute();
/*zoombox.option({})*/
zoombox.option({
